...
很多资料把"丢非参考帧"等同于"丢 B 帧"，这是错的。I/B/P 描述的是
**这一帧自己怎么预测**（帧内 / 前向 / 双向），由 slice header 的
`slice_type` 决定；参考性描述的是**别的帧是否引用它**，由 NAL 头决定。
两者组合出的四个象限在真实码流里都存在：
...
用一个典型的 mini-GOP（`bf=3` + b-pyramid，x264 默认配置）看依赖关系，
箭头表示"引用"：

```mermaid
flowchart LR
    B1["B1<br/>非参考"] --> I0["I0<br/>参考"]
    B1 --> B2["B2<br/>参考B(pyramid)"]
    B2 --> I0
    B2 --> P4["P4<br/>参考"]
    B3["B3<br/>非参考"] --> B2
    B3 --> P4
...
- 显示顺序 `I0 B1 B2 B3 P4`，解码顺序 `I0 P4 B2 B1 B3`；
- **B2 是"参考 B"**：B1 和 B3 都引用它，丢了必花屏；
- B1、B3 没有入边之外的出度——没人引用，随便丢。
...
> 顺带说明收益上限：b-pyramid 开启时连续 3 个 B 里只有 2 个非参考，
> 典型点播流的非参考帧占比在 1/3 ~ 1/2 之间；关闭 b-pyramid 的流
> （所有 B 都非参考）收益最大。
